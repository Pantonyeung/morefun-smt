...
export default function OperationsApp() {
  ...
  const visibleProducts = useMemo(() => Object.values(products).filter((product) => {
    ...
    return !query || `${product.code} ${product.name} ${product.category}`.toLowerCase().includes(query);
  ...
  const navBadge = (key: ViewKey) => key === 'workbench' ? controller.newNetworkCount + controller.abnormalCount : key === 'dinein' ? controller.occupiedTables : key === 'soldout' ? Object.values(products).filter((p) => p.availability !== 'available').length : 0;
  const currentSerial = String(networkOrders.length + 1).padStart(4, '0');

  return <div className="ops-app">
    <header className="ops-topbar">
      <button className="ops-brand" onClick={() => actions.setView('more')} aria-label="營運總覽"><strong>磨飯</strong><span>SMT</span></button>
      <div className="ops-status-strip"><span className="business-open">營業中</span><span className={health.realtime === 'online' ? 'online' : 'offline'}>{health.realtime === 'online' ? '系統正常' : '離線可工作'}</span><span>流水 #{currentSerial}</span></div>
      <button className="ops-new-orders" onClick={() => actions.setView('workbench')}>新單 <em>{controller.newNetworkCount}</em></button>
      <div className="ops-clock"><strong>{now.toLocaleTimeString('zh-HK', { hour: '2-digit', minute: '2-digit' })}</strong><small>{now.toLocaleDateString('zh-HK', { month: 'numeric', day: 'numeric', weekday: 'short' })}</small></div>
    </header>

    <main className="ops-main">
      {view === 'order' ? <div className="ops-order-layout">
        <aside className="ops-cart">
          <header><div><strong>目前訂單</strong><span>{cartQty} 件</span></div><button className={cart.length || holds.length ? 'enabled' : ''} onClick={() => cart.length ? actions.holdCurrent() : actions.setView('hold')}>{cart.length ? '掛單' : `取單${holds.length ? ` ${holds.length}` : ''}`}</button></header>
          <div className="ops-cart-list">{cart.length ? cart.map((item, index) => { const product = products[item.productId]; return <article key={item.id} className={item.pendingIssues.length ? 'issue' : ''}><i>{index + 1}</i><div className="line"><strong>{product?.code} {product?.name}</strong><b>HK${item.estimatedUnitPrice * item.quantity}</b></div><p>{item.summary || '單點'}{item.pendingIssues.length ? ` · 待補 ${item.pendingIssues.length}` : ''}</p><div className="actions"><button onClick={() => actions.updateQuantity(item.id, item.quantity - 1)}>−</button><span>{item.quantity}</span><button onClick={() => actions.updateQuantity(item.id, item.quantity + 1)}>＋</button><button onClick={() => product && setConfiguring({ product, item })}>修改</button><button className="delete" onClick={() => actions.removeItem(item.id)}>刪</button></div></article>; }) : <Empty title="未有餐點" text="由右邊直接選擇商品" />}</div>
          <label className="ops-note"><span>備註</span><input value={orderNote} onChange={(event) => actions.setOrderNote(event.target.value)} placeholder="客人趕時間、分開包裝、到店致電…" /></label>
        ...
        <section className={`ops-catalog mode-${mode}`}>
          <div className="ops-catalog-head"><div className="ops-search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜尋商品名稱或編號" /></div><span>{visibleProducts.length} 項</span></div>
          <div className="ops-category-grid"><button className={category === '全部' ? 'active' : ''} onClick={() => setCategory('全部')}>全部</button>{categories.map((item) => <button key={item} className={category === item ? 'active' : ''} onClick={() => setCategory(item)}>{item}</button>)}</div>
          <div className="ops-product-grid">{visibleProducts.map((product) => <button key={product.id} className={product.availability !== 'available' ? 'sold' : ''} disabled={product.availability !== 'available'} onClick={() => addProduct(product)}>{mode !== 'text' ? <div className="image">{product.imageUrl ? <img src={product.imageUrl} alt="" /> : <span>{product.code}</span>}</div> : null}<div><strong>{product.code} {product.name}</strong><b>HK${product.price}</b></div></button>)}</div>
        </section>
      </div> : null}
      ...
      {view === 'more' ? <section className="ops-page"><header><div><h1>更多</h1><p>營運、付款、設備、同步及帳號</p></div><span>{device.deviceNumber}</span></header><div className="ops-settings"><button onClick={() => setMode('text')} className={mode === 'text' ? 'active' : ''}>大字模式<small>純文字商品卡</small></button><button onClick={() => setMode('compact')} className={mode === 'compact' ? 'active' : ''}>標準模式<small>小圖＋快速點單</small></button><button onClick={() => setMode('image')} className={mode === 'image' ? 'active' : ''}>大圖模式<small>較大商品圖片</small></button><button onClick={() => actions.setView('hold')}>掛單／取單<small>{holds.length} 張暫存</small></button><button onClick={() => actions.setView('pending')}>付款／資料待補<small>{pendingIssues.length} 項</small></button><button onClick={() => actions.setView('reprint')}>設備與重印<small>打印工作管理</small></button><button onClick={() => setDayClose(true)}>日結／點算錢箱<small>完成今日營運</small></button><button disabled>報表<small>下一階段接入</small></button><button disabled>同步與異常<small>{health.realtime === 'online' ? '系統正常' : '需要跟進'}</small></button>{config.mode !== 'demo' ? <button onClick={() => void actions.logout()}>帳號與裝置<small>登出目前裝置</small></button> : null}</div></section> : null}
      {view === 'soldout' ? <section className="ops-page"><header><div><h1>售罄管理</h1><p>即日售罄翌日 05:00 自動恢復</p></div><span>{Object.values(products).filter((p) => p.availability !== 'available').length} 項停止</span></header><div className="ops-soldout">{Object.values(products).map((product) => <button key={product.id} className={product.availability !== 'available' ? 'active' : ''} onClick={() => actions.toggleSoldOut(product.id)}>{product.code} {product.name}<b>{product.availability === 'available' ? '可售' : '售罄'}</b></button>)}</div></section> : null}
      {view === 'reprint' ? <section className="ops-page"><header><h1>設備與重印</h1><span>最近訂單</span></header><div className="ops-hold-grid">{networkOrders.map((order) => <article key={order.id}><strong>{order.orderNo}</strong><span>{sourceLabel(order.source)}</span><b>HK${order.total}</b><button onClick={() => void actions.requestReprint(order.id, '重印管理')}>選擇重印</button></article>)}</div></section> : null}
    </main>
...
function OpeningFloat({ onOpen }: { onOpen: (amount: number) => void }) { const [amount, setAmount] = useState(String(loadSuggestedOpeningFloat())); return <main className="ops-opening"><section><div className="ops-logo">磨飯 <small>SMT</small></div><h1>今日開箱備用金</h1><p>昨日保留金額已自動帶入；如實際不同可直接修改。</p><label>HK$ <input autoFocus inputMode="numeric" value={amount} onChange={(event) => setAmount(event.target.value)} /></label><button onClick={() => onOpen(Math.max(0, Number(amount || 0)))}>確認開市</button></section></main>; }
function OrderBoard({ board, busyKey, onAccept, onReprint }: { board: { new: NetworkOrder[]; active: NetworkOrder[]; exception: NetworkOrder[] }; busyKey: string; onAccept: (order: NetworkOrder) => void; onReprint: (order: NetworkOrder) => void }) { return <section className="ops-page ops-board-page"><header><div><h1>訂單</h1><p>新單 → 已接單 → 可取餐</p></div><span>新單 {board.new.length}｜需處理 {board.exception.length}</span></header><div className="ops-board"><OrderColumn title="新單" tone="new" orders={board.new} render={(order) => <OrderTile order={order} action={<button disabled={busyKey.includes(order.id)} onClick={() => onAccept(order)}>馬上處理</button>} />} /><OrderColumn title="已接單" tone="active" orders={board.active} render={(order) => <OrderTile order={order} action={<button onClick={() => onReprint(order)}>重印／查看</button>} />} /><OrderColumn title="需處理" tone="exception" orders={board.exception} render={(order) => <OrderTile order={order} action={<button onClick={() => onReprint(order)}>處理異常</button>} />} /></div></section>; }
