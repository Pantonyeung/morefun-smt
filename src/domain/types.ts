...
export interface StaffProfile {
  uid: string;
  role: 'smt' | 'admin' | 'smm' | 'printer' | 'unknown';
  active: boolean;
  displayName: string;
  deviceNumber?: string;
}
...
export interface CartSelections {
  ...
  riceAmount?: 'normal' | 'less' | 'half' | 'more';
  noEgg?: boolean;
  riceBallId?: string;
  secondRiceBallId?: string;
  snackId?: string;
  drinkId?: string;
  noDrink?: boolean;
  saucePrimary?: string;
  ...
  doubleSauce?: boolean;
  ice?: 'normal' | 'less' | 'no_ice';
  sweetness?: 'normal' | 'less' | 'no_sugar';
}
...
export interface TablePaymentGroup {
  id: string;
  itemIds: string[];
...
export interface TableSession {
  ...
  paymentGroups: TablePaymentGroup[];
  openedAt?: string;
