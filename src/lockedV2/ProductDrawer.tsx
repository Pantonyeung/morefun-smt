...
export function ProductDrawer({ product, catalog, item, initialMode = item ? 'quick' : 'full', onSave, onClose }: {
  ...
  return <div className="v2-drawer-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="v2-product-drawer" role="dialog" aria-modal="true" aria-label={`${product.code} ${product.name}`}>
      <header>
        <div><span>{item ? '快捷修改' : '商品設定'}</span><h2>{product.code} {product.name}</h2><p>{preview.summary || '單點'} · HK${preview.estimatedUnitPrice}</p></div>
      ...
      {mode === 'quick' ? <section className="v2-quick-edit">
        ...
        <button className="v2-full-detail" onClick={() => setMode('full')}>查看完整詳情</button>
      </section> : <div className="v2-detail-scroll">
        <RuleSection title="Required" subtitle={`${preview.pendingIssues.length}項未完成`} tone={preview.pendingIssues.length ? 'warning' : 'success'}>
        ...
        <RuleSection title="Pool" subtitle="共享候選">
          ...
          </> : <p className="v2-empty-rule">此商品沒有共享候選要求。</p>}
        </RuleSection>

        <RuleSection title="Link Up" subtitle="加配及連動差額">
          ...
          {!['rice_ball', 'custom_set', 'salad', 'snack'].includes(product.ruleKind) ? <p className="v2-empty-rule">此商品沒有Link Up選項。</p> : null}
        </RuleSection>
...
function QuickControls({ product, selections, patch, drinks }: { product: Product; selections: CartSelections; patch: (value: Partial<CartSelections>) => void; drinks: Product[] }) {
  ...
  return <p className="v2-empty-rule">常用修改已保留；完整選項請進入完整詳情。</p>;
}
